...
!!! note "To get test access"
    ...
    !!! info "Outbound IP change — October 2026"
        NoGi's outbound IP addresses are changing. See [Outbound IP address change](../outbound-ip-change.md) for details.
...
|---|---|---|
| Full dataset retrieval | Norsk Gjeldsinformasjon triggers a `getalldata` call against your test endpoint | HTTP 200 with all test customers returned correctly |
| Data validation | Internally validate data before the call | Norsk Gjeldsinformasjon accepts without validation errors |
...
| Test | Pass criteria |
|---|---|
| New loan account | Norsk Gjeldsinformasjon receives push; subsequent getDataForSSN returns the new loan |
| Credit limit change | Updated limit reflected in Norsk Gjeldsinformasjon |
...
3. **Norsk Gjeldsinformasjon configures production** — endpoints, IPs, and certificate registered
